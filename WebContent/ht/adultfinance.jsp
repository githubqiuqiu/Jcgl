...
function add(){
	...
	$('#tab').datagrid({
		...
		url:"seladultfinance?astudentno="+astudentno+"&aname="+aname+"&teacher="+teacher+"&status="+status+"&types="+types,
		...
		columns:[[
			...
			{field : "acaname", title :"院校名称" ,width:80},
			{field : "specname", title :"专业名称",width:150},
			{field : "gradation", title : "层次",width:150},
...
//编辑学生信息(打开编辑窗口)
function editadult(id){
		//alert("进了编辑");
	  $('#winedit').window('open');
	  $('#winedit').window('refresh','beforeadultedit.action?id='+id);
}
...
function edits(id){
	  $('#winedit').window('open');
	  $('#winedit').window('refresh','beforeadultfinanceedit.action?id='+id);
}
...
<body class="easyui-layout">
	...
	<div data-options="region:'center'" style="height:50px">
		<table id="tab" fit="true" class="easyui-datagrid" style="padding:0px" title="成考缴费信息表"></table>
	</div>
	...
	<div id="winadd" class="easyui-window" 
		...
		title="编辑" 
		data-options="modal:true,closed:true,collapsible:false,minimizable:false,maximizable:false,border:false" 
		style="width:500px;height:240px;padding:5px;">
		</div> 
