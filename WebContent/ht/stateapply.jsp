...
function add(){
	...
	$('#tab').datagrid({
		...
		url:"selstate?astudentno="+astudentno+"&aname="+aname+"&teacher="+teacher,
		...
		columns:[[
			...
			{field : "sstudentno", title : "学生编号" ,width:80 },
			{field : "sname", title :"学生姓名" ,width:80},
			{field : "username", title :"老师名称" ,width:80},
			{field : "idnumber", title :"身份证号码",width:150},
			{field : "sphone", title : "手机号码",width:150},
					
			{field : "acaname", title : "报考院校",width:100},
			{field : "specname", title : "专业",width:80},
			{field : "sgradation", title : "报考层次",width:80},
			{field : "ssystem", title : "学制",width:80},
			{field : "stype", title : "报名类型",width:80,
			...
			{field : "sdata", title : "报名时间",width:150},
			{field : "caozuo", title :"操作"  ,width:150 ,
				formatter : function(value,row, rec) {
				var btn ='' ;
				btn+="<a class='edit'  href='#' onclick='edits("+row.sid+")'>编辑</a>";
...
function adddata(){
	$('#winadd').window('open');
	$('#winadd').window('refresh','stateadd.action');
}
...
function edits(id){
	  //alert("进了编辑");
	  $('#winedit').window('open');
	  $('#winedit').window('refresh','beforestateedit.action?id='+id);
}
